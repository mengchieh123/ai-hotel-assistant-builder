// booking_controller.js (V8.1 - 完整修正與流程完善版)
...
    const data = session.collectedData;
    cleanRichCard(data); // 🎯 修正：進入計算前清除 Rich Card
    const { roomType, checkInDate, nights, roomCount, adultCount } = data;
    
...
    const data = session.collectedData;
    cleanRichCard(data); // 進入加購頁面時清除，避免舊的會員卡片殘留
    
    try {
...
// 🎯 修正：取代原 validateContactInfoLogic，專門用於檢查實體和清理卡片
...
function generatePaymentOptionsLogic(session) {
    ...
    const paymentOptions = {
        type: 'button_list',
        buttons: [
            { text: '信用卡支付', value: '信用卡', intent: 'correction' },
            { text: '銀行轉帳', value: '銀行轉帳', intent: 'correction' },
            { text: '現場支付', value: '現場支付', intent: 'correction' }
        ]
    };

    data.customRichCard = paymentOptions;
    
    // 讓 Rule Engine 決定 Prompt，這裡只負責生成卡片
    return { isHandled: true };
}

// --- 16. 生成訂單摘要 (generateOrderSummary) ---
async function generateOrderSummaryLogic(session) {
    // 🎯 修正：在最終摘要頁面清除舊卡片，生成新的確認按鈕
    const data = session.collectedData;
    cleanRichCard(data);
...
        type: "button_list",
        buttons: [
            { text: "✅ 確認並提交訂單", value: "確認", intent: "affirm" },
            { text: "✏️ 修改房型或日期", value: "我要修改房型", intent: "correction" },
...
                : '付款連結已透過郵件寄給您。';
            await unlockInventory(session);
            // 🎯 修正：導向 final_confirmation 狀態
            return { isHandled: true, nextStep: 'final_confirmation' };
        } else {
            return { 
                isHandled: true, 
...
    static checkContactInfo = checkContactInfoLogic;
    static handleSpecialRequests = handleSpecialRequestsLogic; 
    // 🎯 新增：付款選項生成
    static generatePaymentOptions = generatePaymentOptionsLogic;
    // 🎯 修正：使用 generateOrderSummaryLogic
