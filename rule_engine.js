...
// 載入 Flow Config - ✅ 保持使用同步 FS 載入 JSON (繞過 assert 錯誤)
let flowConfig;
try {
...
class RuleEngine {
    ...
    static async executeRules(message, sessionId) {
        ...
        try {
            ...
            // 1. 意圖分類與實體抽取
            const classificationResult = SmartIntentClassifier.classify(message, flow); // ✅ 使用修正後的命名導入
