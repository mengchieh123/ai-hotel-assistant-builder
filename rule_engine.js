...
class RuleEngine {
    ...
    static sanitizeEntities(extractedEntities) {
        ...
        // 🚨 修正：移除對 '登入' 的過濾，讓 'member_login' 意圖能被 P:100 規則捕獲。
        // 保留 '完成' 和 '下一步' 在黑名單，防止其誤判為 name 實體
        const suspiciousNameKeywords = ['我要', '我想', '改日', '吸菸', '加價', '多加', '繼續', '訂接', '確認', '行政套房', '豪華客房', '標準雙人房', '家庭四人房', '不客氣', '共住', '我要改', '間入住人', '查看加購', '跳過', '完成', '下一步', '修改', '幫我訂', '訂房']; 
    ...
    static async executeRules(intents, session, message, extractedEntities) {
        const rules = [
            ...
            { fn: this.emergencyRule, name: '緊急事件規則', priority: PRIORITY.EMERGENCY }, // P:105
            { fn: this.roomLimitRule, name: '房間數量上限規則', priority: PRIORITY.BOOKING_FLOW.ROOM_LIMIT }, // P:103
            { fn: this.generalInquiryOverrideRule, name: '通用查詢覆蓋規則', priority: PRIORITY.GENERAL_INQUIRY_OVERRIDE }, // P:104
            
            // 🚨 新增會員登入規則 (P:100)，確保流程中斷時能正確處理登入
            { fn: this.memberLoginRule, name: '會員登入規則', priority: PRIORITY.BOOKING_FLOW.MEMBER_LOGIN }, 
    ...
    static memberLoginRule(intents, session) {
        // 🚨 條件：偵測到明確的會員登入意圖 (member_login)
        if (intents.includes('member_login')) {
            const nextStateKey = 'handle_member_login';
            
            // 強制清除 handler 紀錄，確保登入 Handler (handleLoginAttempt) 會被執行。
            resetHandlerExecution(session); 
            
            return {
                shouldProcess: true,
                priority: PRIORITY.BOOKING_FLOW.MEMBER_LOGIN, // P:100
                response: '正在為您處理會員登入與價格切換...',
                nextStep: nextStateKey,
                richCard: null,
                allowGeminiCall: false
            };
    ...
    static async bookingFlowRule(intents, session, message) {
        ...
        while (flow.states[nextStateKey]?.handler && !hasExecutedHandler(session, nextStateKey)) {
            ...
            if (handlerResult.isHandled) {
                ...
                // 🚨 關鍵修正點：鎖定透明化/ Handler 處理完成的回覆 (lockInventory 成功後會在此返回)
