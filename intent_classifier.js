// intent_classifier.js (V6.1 - 強化中文數字和晚數解析)
...
        this.extractAddons(normalizedMessage, extractedEntities, flowConfig);
...
        // 原版: const nightsMatch = message.match(/(\d+)(晚|天|night|days?)/);
...
        // 🏆 修正: 房間數匹配，支持中文數字
        // 原版: const roomCountMatch = message.match(/(\d+)(間房|間|rooms?)/);
...
        // 🏆 修正: 兒童數匹配，支持中文數字
        // 原版: const childCountMatch = message.match(/(\d+)(個?兒童|小孩|kids?|children)/);
...
        // 🏆 修正: 成人數匹配，支持中文數字
        // 原版: const adultCountMatch = message.match(/(\d+)(個?成人|大人|adults?)/);
...
        // 原版: const totalMatch = message.match(/(\d+)位/);
...
        // [您的 extractContactInfo 函數代碼 (保持不變)]
...
        // 🚨 修正：強化 Name 匹配，只在有引導詞或匹配條件更嚴格時才提取，避免將房型誤判為姓名
...
        // [您的 extractMemberInfo 函數代碼 (保持不變)]
