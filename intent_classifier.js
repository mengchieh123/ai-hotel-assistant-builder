// intent_classifier.js (V6.0 - 完整修正版)
...
class SmartIntentClassifier {
    ...
    // --- 實體抽取輔助函數 (為簡潔，僅保留修改部分) ---

    static extractDateEntities(message, entities) {
        // [您的 extractDateEntities 函數代碼 (保持不變)]
    ...
    static extractCountEntities(message, entities) {
        ...
        if (totalMatch && parseInt(totalMatch[1]) > 0 && !entities.adultCount && !entities.childCount) {
    ...
    static extractContactInfo(message, entities) {
        // [您的 extractContactInfo 函數代碼 (優化 Name Regex)]
        ...
        // 注意：為了防止誤判，此版本**移除了**沒有引導詞時的姓名提取，因為這在您先前的日誌中造成了「豪華客房」被誤判為姓名的問題。
    }
