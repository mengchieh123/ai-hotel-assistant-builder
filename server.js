...
// --- API Key 和配置 ---
// 從環境變數中讀取 Key，如果未設定，使用一個無效的 Key 進行本地測試
...
        if (intents.length > 1) {
            // 這裡可以選擇調用 AI，但為了避免不確定性，保留靜態串接邏輯
...
        if (!apiKey || apiKey === "AIzaSyBMOdSKtUDMcwXXbg_Zu0cXMOPedmyr_Q0") {
            console.warn("[Gemini API] API Key is empty or placeholder. Using fallback response.");
...
        const payload = {
            // 將 systemPrompt 放在 contents 的開頭
            contents: [{ role: "system", parts: [{ text: systemPrompt }] }].concat(contents),
            generationConfig: {
                maxOutputTokens: 500,
                temperature: 0.7
            }
        };
        ...
        // 修正：Gemini API 不支援 "system" role 在 chat context 中，應使用 "model" role
        // 讓我們移除 system role 的插入，改為使用 'contents' 的形式，並確保歷史對話正確映射。
        // （由於您的原始程式碼沒有使用 Gemini.js 庫，我們將使用簡單的 `contents` 傳輸，並依賴模型自身理解上下文）
...
        // 這個函數現在會作為 API Key 無效時的 LLM Fallback
